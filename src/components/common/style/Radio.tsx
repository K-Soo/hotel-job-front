...
export default function Radio({
  ...
  return (
    <S.Radio $margin={margin} style={style} className={className}>
      <S.RadioContainer className="wrapper" $fontSize={fontSize} $active={checked}>
        <input
          id={`radio-${value}`}
          type="radio"
          onChange={onChange}
...
const S = {
  Radio: styled.div<{ $margin?: string; $fontSize?: string }>`
  ...
  `,
  RadioContainer: styled.div<{ $active?: boolean; $fontSize?: string }>`
    display: flex;
    align-items: center;
    justify-content: space-between;
    line-height: 1.2;
